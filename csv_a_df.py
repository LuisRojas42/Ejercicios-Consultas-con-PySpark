# -*- coding: utf-8 -*-
"""
Created on Mon Jan 10 21:21:19 2022

@author: CONSULTOR

 ../spark2/bin/spark-submit csv_a_df.py 2>prg.out
...
listening_df = sqlContext.read.format("com.databricks.spark.csv").option("inferSchema", True).option("header", True).load("../listenings.csv")
inicio = time.time()
listening_df.show()
final = time.time()
print("Tiempo de impresion: ", (final-inicio))

genre_df = sqlContext.read.format("com.databricks.spark.csv").option("inferSchema", True).option("header", True).load("../genre.csv")
inicio = time.time()
genre_df.show()
final = time.time()
print("Tiempo de impresion: ", (final-inicio))
